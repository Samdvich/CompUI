  ...
  <body>
    ...
    if (isset($_POST['create-submit'])) {
      ...
      elseif ((strpos($_POST['user-field'], " ") == true) || (strpos($_POST['password-field'], " ") == true)) {
        ...
        elseif ($_POST['account'] == 'admin' || 'teacher') {
          if ($secure = $conn->prepare('INSERT INTO `accounts` (house, type, email, password) VALUES(?, ?, ?, ?);')) {
            $password = password_hash($_POST['password-field'], PASSWORD_BCRYPT);
            $account = $_POST['account'];
            $email = $_POST['user-field'];
            $secure->bind_param('ssss', $account, $account, $email, $password);
            $secure->execute();
            $secure->close();
            echo "<p id='warning-message'>Complete - <a href='accounts.php?choice=create-button' style='color: blue;'>Return</a></p>";
            $autorefresh;
    ...
    if (isset($_POST['delete-submit'])) {
      ...
      elseif (strpos($_POST['user-field'], " ") == true) {
        echo "<p id='warning-message'>Please ensure no spaces in email or password - <a href='accounts.php?choice=create-button' style='color: blue;'>Retry</a></p>";
        $autorefresh;
      }
      else {
        $secure = $conn->prepare('SELECT email, type, house FROM accounts WHERE email = ?');
    ...
    <style>
      ...
      #home { grid-column: 1; color: white; margin: auto; margin-left: 10%; }
      
      #heading { grid-column: 2; font-size: 350%; color: white; margin: auto; }
      
      .import_csv { grid-row: 2; grid-column: 2; height: 50px; width: 35%; background-color: white; border-radius: 20px; margin: auto; margin-right: 10%; display: grid; grid-template-columns: 75% 25%; }
      ...
      .frame { grid-row: 2; grid-column: 2; display: grid; grid-template-columns: 1fr; grid-template-rows: 1fr; height: 90%; width: 95%; margin: auto; margin-left: 0; border: 2px solid black; }
      ...
      .viewaccounts { grid-column: 1; margin: auto; margin-top: 5%; overflow: scroll; }
